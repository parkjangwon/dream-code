...
function buildCronJob(input: CronJobInput, timestamp: string, now: Date): CronJob {
  const job: CronJob = {
    id: randomUUID(),
...
function normalizeNextRun(job: CronJob, now: Date): CronJob {
  if (!job.enabled) {
...
function definedPatch(patch: CronJobPatch): CronJobPatch {
  return Object.fromEntries(Object.entries(patch).filter((entry) => entry[1] !== undefined));
}

function singleMatch<T>(matches: readonly T[], label: string, query: string): T {
  if (matches.length === 1) {
    const [match] = matches;
    if (match !== undefined) {
      return match;
