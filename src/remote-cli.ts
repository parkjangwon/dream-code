...
function remoteRuntimeDir(root: string): string {
...
async function writeReady(root: string, ready: RemoteReady): Promise<void> {
  await mkdir(remoteRuntimeDir(root), { recursive: true, mode: 0o700 });
  await writeFile(remoteReadyPath(root), `${JSON.stringify(ready, null, 2)}\n`, "utf8");
}

async function readRemoteReady(root: string): Promise<RemoteReady | undefined> {
  try {
...
async function waitForReady(root: string): Promise<RemoteReady> {
...
async function removeRuntimeFiles(root: string): Promise<void> {
  await rm(remoteReadyPath(root), { force: true });
}

function removeRuntimeFilesSync(root: string): void {
  rmSync(remoteReadyPath(root), { force: true });
}

function stopTailscaleServeSync(port: number): void {
